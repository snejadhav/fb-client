...
// Adding Data
router.post('/tododata',function(req, res) {

		var myInput = req.body.myInput;
		var title = new Todotask({
						title: myInput
					});
		  var new_task = new Todotask(title);

		  new_task.save(function(err, task) {
		    if (err){
		      res.send(err);
		      req.flash('success_msg', 'error'+err);}else{
		    // res.json(task);
...
router.get('/todo',function(req, res){
	Todotask.find({}, function(err, reply) {
				if (err){
					res.send(err);
					}
			 res.json(reply);
